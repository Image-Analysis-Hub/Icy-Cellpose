		...
		ThreadUtil.bgRun( () -> {
			...
			for ( final IcyBufferedImage plane : sequence.getAllImage() )
			{
				try
				{
					 ...
					 * endUpdate (all in a try/finally block), so that we only
					 * display the display (which takes time) once the edits of
					 * a plane are over.
					 *
					 * We could also have chosen to put the beginUpdate() before
					 * editing the sequence to have only one display update.
					 */

					plane.beginUpdate();

					/*
					 * We pedestriantly iterate pixel by pixel and recopy the
					 * value of the every 7th pixel on the 2 that follows.
					 */
